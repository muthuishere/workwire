...
func TestParseInstallFlags(t *testing.T) {
	cases := []struct {
		...
		// Auto-join is gone: joining is opted into by the repo's own
		// CLAUDE.md / AGENTS.md, never by an installed hook.
		{name: "auto is no longer a flag", args: []string{"--auto"}, wantErr: true},
		{name: "off is no longer a flag", args: []string{"--off"}, wantErr: true},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if f.skills != c.wantSkills || f.service != c.wantService || f.dir != c.wantDir {
				t.Fatalf("parsed %+v want skills=%v service=%v dir=%q",
					f, c.wantSkills, c.wantService, c.wantDir)
			}
		})
	}
}
